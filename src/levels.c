...
//		ball[i].bonus = BONUS_LIFE;
	...
	if (n==6)
	{
		createPlateforme(81.0-32.0,65.0,PF_MOYEN_INCASSABLE);
		createPlateforme(297.0-32.0,65.0,PF_MOYEN_INCASSABLE);
		createPlateforme(185.0-32.0,65.0,PF_MOYEN_CASSABLE);
		i=createBall(154.0-32.0,16.0,BIG,LEFT,DOWN);
		ball[i].bonus = WEAPON_DOUBLE_SHOOT;
		createBall(192.0-32.0,16.0,BIG,RIGHT,DOWN);
	}
	if (n==7)
	{
	...
	if (n==8)
	{
		...
		i=createBall(150.0-32.0,79.0,NORMAL,RIGHT,DOWN);
		ball[i].bonus = BONUS_FREEZE;
		i=createBall(209.0-32.0,79.0,NORMAL,RIGHT,DOWN);
		ball[i].bonus_parent=getRandomBonus();
	}
