...
//	}
	...
	if (player.posx<8) player.posx=8;
	if (player.posx>283) player.posx=283;

	if (player.posy>200-32) player.posy=200-32;
 ...
 // touché !
	for (i=0;i<MAX_BALL;i++)
	{
		if (ball[i].utilise==1)
		{
			if (isCollide(player.xbox+player.posx,player.ybox+player.posy,player.hbox,player.lbox,ball[i].xbox+ball[i].posx,ball[i].ybox+ball[i].posy,ball[i].hbox,ball[i].lbox)==1)
			{
				...
				if (ball[i].utilise==1)
				{
					if (isSpriteCollide(player.spriteno,ball[i].spriteno)==1)
					{
						if (player.bonus_protection==1)
						{
							...
							{
								player.bonus_protection_timer=30;	
							}
							explodeABall(i);
						}
						// si on est pas deja mort
