...
/*
 Copie … l'‚cran l'image de la balle num‚ro i
*/
void showBall(int i)
{
    if (ball[i].utilise==1)
...
void explodeABall(int a)
{
	...
	ball[tmp2].hauteurmax_cpt = ball[tmp2].hauteurmax-10;
	ball[tmp2].vel=2.0;
	...
	if (ball[a].bonus!=0)
	{
       createBonus(ball[a].bonus,(int)ball[a].posx+5,(int)ball[a].posy+5);
	}
			...
			if (shoot[p].utilise==1)
			{
				...
				if (collide!=0) 
				{
					...
					if (ball[i].type<MICRO) 
					{
						...
						ball[tmp].hauteurmax_cpt = ball[tmp].hauteurmax-10;
						ball[tmp].vel=2.0;
