...
int getRandomBonus()
{
	...
	// evilo - never reach this place anyway
	return -1;
}
...
void updateBonus(int i)
{
	...
	if (bonus[i].utilise==1)
	{
		...
		{
			bonus[i].posy=200-bonus[i].hbox;
			bonus[i].etat = STOP;
		}

		if (bonus[i].etat==DOWN) bonus[i].posy+=1;
...
//		else if (bonus[i].type==BONUS_PROTECTION) GpTransLRBlt(NULL, &gpDraw[nflip], bonus[i].posx, bonus[i].posy, 18,18, (unsigned char*)sprite1, 158, 191, 320, 240,INVISIBLE_COLOR);
		...
		if (bonus[i].type==WEAPON_DOUBLE_SHOOT) blitImageToScreen(1,53,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
		else if (bonus[i].type==WEAPON_GLUE_SHOOT) blitImageToScreen(1,11,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
		else if (bonus[i].type==BONUS_BOOM) blitImageToScreen(1,116,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
		else if (bonus[i].type==BONUS_FREEZE) blitImageToScreen(1,32,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
		else if (bonus[i].type==BONUS_LIFE) blitImageToScreen(1,74,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
		else if (bonus[i].type==BONUS_PROTECTION) blitImageToScreen(1,158,191,18,18,bonus[i].posx,bonus[i].posy,18,18,320,240);
